    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
                @lang('crud.galleries.index_title')
            </h2>
            ...
            <a href="{{ route('galleries.create') }}"
                ...
                <i class="icon ion-md-add text-base"></i> New Gallery
            </a>
            @endcan
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 px-5 py-4">
                <form method="GET" class="flex flex-wrap gap-3 items-end">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if($search ?? '')
                        <a href="{{ route('galleries.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 px-3 py-2 rounded-lg transition">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <p class="text-sm text-gray-500">
                        <span class="font-semibold text-gray-800">{{ $galleries->total() }}</span> records
                        @if($galleries->total() != $galleries->count())
                        &mdash; showing {{ $galleries->firstItem() }}-{{ $galleries->lastItem() }}
                        @endif
                    </p>
                </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-5 py-3">
                                    <x-partials.thumbnail src="{{ $gallery->image ? \Storage::url($gallery->image) : '' }}" />
                                </td>
                                <td class="px-5 py-3.5">
                                    <p class="font-medium text-gray-800">{{ $gallery->title ?? '-' }}</p>
                                </td>
                                <td class="px-5 py-3.5 text-center">
                                    <div class="inline-flex items-center gap-1">
                                        ...
                                        @endcan
                                        @can('view', $gallery)
                                        <a href="{{ route('galleries.show', $gallery) }}" title="View"
                            ...
                            <tr>
                                <td colspan="3" class="px-5 py-16 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        <i class="icon ion-md-images text-5xl mb-3"></i>
